...
public class ServiceEntityManager
{
    public static void insert(ServiceEntity service) throws SQLException
    {
        ...
        {
            String sql = "INSERT INTO Service(Title, Duration, Cost, Discount, MainImagePath, Description) VALUES(?,?,?,?,?,?)";
            ...
            ps.setString(6, service.getDesc());
    ...
    public static void update(ServiceEntity service) throws SQLException
    {
        try(Connection c = App.getConnection())
        {
            String sql = "UPDATE Service SET Title=?, Duration=?, Cost=?, Discount=?, MainImagePath=?, Description=? WHERE ID=?";
